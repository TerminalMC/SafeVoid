<div align="center"><center>

## Archival Notice

This project will no longer receive normal updates, but requests may still be considered.

<img alt="Icon" width=100 src="https://raw.githubusercontent.com/TerminalMC/SafeVoid/HEAD/common/src/main/resources/assets/safevoid/icon.png">

## SafeVoid

Prevents void damage and wraps the world vertically.

[![Environment](https://img.shields.io/badge/Environment-Server-blue?logo=data:image/png;base64,)]()
[![Latest Minecraft](https://img.shields.io/modrinth/game-versions/XLBH6YfK?label=Latest%20Minecraft&color=%2300AF5C&logo=data:image/png;base64,)](https://modrinth.com/mod/XLBH6YfK/versions)

[![Loader](https://img.shields.io/badge/Available%20for-Fabric-dbd0b4?logo=data:image/png;base64,)](https://fabricmc.net/)
[![Loader](https://img.shields.io/badge/Available%20for-NeoForge-f16436?logo=data:image/png;base64,)](https://neoforged.net/)

[![Download on Modrinth](https://img.shields.io/modrinth/dt/XLBH6YfK?label=Download%20on%20Modrinth&logo=modrinth&logoColor=%2300AF5C)](https://modrinth.com/mod/XLBH6YfK)
[![Download on GitHub](https://img.shields.io/github/downloads/TerminalMC/SafeVoid/total?label=Download%20on%20GitHub&logo=github&logoColor=white)](https://github.com/TerminalMC/SafeVoid)

</center></div>

### About

This mod was created mainly to allow unrestricted elytra flight in the end void, because 
[Complementary Shaders](https://modrinth.com/shader/complementary-unbound) & 
[Euphoria Patches](https://modrinth.com/mod/euphoria-patches) (shown below) are awesome.

In addition to blocking void damage, it wraps the world so if you go far enough down you'll end up back at the top.

It's enabled in all dimensions, and by default only works for players. You can enable it for all `LivingEntity`s and
configure the world wrap via the config file (no config UI, maybe later).

Available commands are `/safevoid enable|disable|reload`

It works if only installed server-side, but if you want to be able to use rockets in the void, it has to be installed 
on the client as well.

<img src="https://raw.githubusercontent.com/TerminalMC/SafeVoid/HEAD/assets/images/end_01.png" style="width:100%;">

### Contact

[![Discord Server](https://img.shields.io/discord/1103153365216669797?logo=discord&label=Discord%20Server&color=%235865F2)](https://discord.terminalmc.dev)

[![GitHub Issues](https://img.shields.io/github/issues/TerminalMC/SafeVoid?logo=github&label=GitHub%20Issues)](https://github.com/TerminalMC/SafeVoid/issues)

[![License](https://img.shields.io/github/license/TerminalMC/SafeVoid?label=License&logo=github&logoColor=white)](https://github.com/TerminalMC/SafeVoid/blob/HEAD/LICENSE.txt)
